...
void TVE::Pipeline::CreateGraphicsPipeline
(
	const std::string& vertFilepath, 
	const std::string& fragFilepath, 
	const PipelineConfigInfo& configInfo
)
{
	...
	shaderStages[0].pName				= "main"; // name of the entry function in vertex shader
	...
	shaderStages[1].pName = "main"; // name of the entry function in vertex shader
	shaderStages[1].flags = 0;
	shaderStages[1].pNext = nullptr;
	shaderStages[1].pSpecializationInfo = nullptr;

	VkPipelineVertexInputStateCreateInfo vertexInputInfo{}; // describe how to interpret vertex buffer data
	vertexInputInfo.sType								= VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
	vertexInputInfo.vertexAttributeDescriptionCount		= 0;	// 0 since we currently hard coded the shader values, will be modified
	vertexInputInfo.vertexBindingDescriptionCount		= 0;	// 0 since we currently hard coded the shader values, will be modified
	vertexInputInfo.pVertexAttributeDescriptions		= nullptr;
	vertexInputInfo.pVertexBindingDescriptions			= nullptr;

	// Limit viewport and scissor count to 1 (preference, not required) enabling multiple requires in depth analysis of devices being used
	VkPipelineViewportStateCreateInfo viewportInfo{}; // this set pNext and flags = 0
...
void TVE::Pipeline::DefaultPipelineConfigInfo(PipelineConfigInfo configInfo, uint32_t width, uint32_t height)
{
	...
	// viewport describes the transformation between pipeline output and target image
	...
	configInfo.rasterizationInfo.depthClampEnable			= VK_FALSE; // forces z component to between 0 and 1
	configInfo.rasterizationInfo.rasterizerDiscardEnable	= VK_FALSE; // only enable if you want to use first stages of graphics pipeline
	configInfo.rasterizationInfo.polygonMode				= VK_POLYGON_MODE_FILL;
	configInfo.rasterizationInfo.lineWidth					= 1.0f;
	configInfo.rasterizationInfo.cullMode					= VK_CULL_MODE_NONE; // front face or back face culling // none for now, will be modified
	configInfo.rasterizationInfo.frontFace					= VK_FRONT_FACE_CLOCKWISE;
	configInfo.rasterizationInfo.depthBiasEnable			= VK_FALSE; // advanced stuff
